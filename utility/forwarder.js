...
async function loadScheduleFromDrive() {
    try {
        ...
        if (response.data) {
            lastSendTime = new Date(response.data);
...
async function saveMediaToDrive(buffer, mimeType, extension) {
    ...
    const response = await drive.files.create({ requestBody: requestBody, media: media, fields: 'id' });
